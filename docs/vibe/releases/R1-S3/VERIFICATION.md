...
[来源登记](SOURCE_REGISTRY.json)初始保存四个仓库的固定提交，2026-09-25 增列 Proof-of-Patch；记录许可状态、公开材料入口及未核实字段。首轮依据各项目原始 GitHub 仓库和论文入口核对，未下载数据集到本项目，也没有给真实样本编造源码摘要或人工真值。后续十项待审名单记录从固定上游源码计算的真实字节摘要，仍未准入人工真值。
